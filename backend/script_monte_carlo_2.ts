...
async function runMonteCarlo() {
  console.log("🚀 Iniciando Simulação de Monte Carlo V2...");
  ...
  const manager = await db.insert(users).values({
    name: "Gestor Monte Carlo V2",
    email: `gestor_mc2_${runId}@teste.com`,
    passwordHash: defaultPassword,
    role: "MANAGER"
  }).returning();
  const managerId = manager[0].id;

  // 2. Criar 100 Colaboradores
  console.log("👥 Criando 100 Colaboradores e Personas...");
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const user = await db.insert(users).values({
      name: fullName,
      email: `collab_mc2_${i+1}_${runId}@teste.com`,
      passwordHash: defaultPassword,
      role: "COLLAB"
    }).returning();
  ...
  // 3. Estruturar Desafios (100 pessoas * 10 desafios = 1000 participações)
      ...
      for (let pid of currentPersonas) {
        for (let j = 0; j < axisIds.length; j++) {
          ...
          await db.insert(evaluations).values({
            challengeId, cycleId, personaId: pid, axisId: axId, managerId, rating, observation: "Feedback simulado MC2"
          });
        }
  ...
  console.log("✅ Nova Simulação (100 Colab/Multi-ciclos) finalizada com sucesso!");
  process.exit(0);
}
